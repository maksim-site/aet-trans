...
const serviceTransport = document.querySelector("[data-service-transport]");

if (serviceTransport) {
  const serviceTransportButtons = [...serviceTransport.querySelectorAll("[data-transport-key]")];
  const serviceTransportIcons = [...serviceTransport.querySelectorAll("[data-transport-icon]")];
  const serviceTransportCaptions = [...serviceTransport.querySelectorAll("[data-transport-caption]")];
  const serviceTransportPath = serviceTransport.querySelector("[data-transport-path]");
  const serviceTransportVehicle = serviceTransport.querySelector("[data-transport-vehicle]");
  const serviceTransportDurations = { road: 1050, rail: 1250, sea: 1400, air: 780 };
  const serviceTransportLandingFade = 180;
  let serviceTransportDuration = serviceTransportDurations.road;
  let serviceTransportFrame = 0;
  let serviceTransportStartedAt = 0;

  const positionServiceTransportVehicle = (progress) => {
    if (!serviceTransportVehicle || !serviceTransportPath || typeof serviceTransportPath.getTotalLength !== "function") return;
    const length = serviceTransportPath.getTotalLength();
    if (!length) return;
    const point = serviceTransportPath.getPointAtLength(Math.max(0, Math.min(1, progress)) * length);
    serviceTransportVehicle.setAttribute("transform", `translate(${point.x.toFixed(2)} ${point.y.toFixed(2)})`);
  };

  const animateServiceTransportVehicle = (time) => {
    if (!serviceTransportStartedAt) serviceTransportStartedAt = time;
    const elapsed = time - serviceTransportStartedAt;

    if (elapsed <= serviceTransportDuration) {
      const progress = Math.min(1, elapsed / serviceTransportDuration);
      positionServiceTransportVehicle(0.5 - Math.cos(Math.PI * progress) / 2);
      if (serviceTransportVehicle) {
        serviceTransportVehicle.style.opacity = Math.min(1, progress / 0.07).toFixed(3);
      }
    } else {
      const fadeProgress = Math.min(1, (elapsed - serviceTransportDuration) / serviceTransportLandingFade);
      positionServiceTransportVehicle(1);
      if (serviceTransportVehicle) {
        serviceTransportVehicle.style.opacity = (1 - fadeProgress).toFixed(3);
      }
    }

    if (elapsed < serviceTransportDuration + serviceTransportLandingFade) {
      serviceTransportFrame = requestAnimationFrame(animateServiceTransportVehicle);
      return;
    }
    if (serviceTransportVehicle) serviceTransportVehicle.style.opacity = "0";
    serviceTransportFrame = 0;
    serviceTransport.classList.add("is-complete");
  };

  const activateServiceTransport = (transportKey) => {
    serviceTransportButtons.forEach((button) => {
      button.setAttribute("aria-pressed", String(button.dataset.transportKey === transportKey));
    });
    serviceTransportIcons.forEach((icon) => {
      icon.classList.toggle("is-active", icon.dataset.transportIcon === transportKey);
    });
    serviceTransportCaptions.forEach((caption) => {
      caption.classList.toggle("is-active", caption.dataset.transportCaption === transportKey);
    });

    if (serviceTransportFrame) cancelAnimationFrame(serviceTransportFrame);
    serviceTransportFrame = 0;
    serviceTransportStartedAt = 0;
    serviceTransportDuration = serviceTransportDurations[transportKey] ?? 1050;

    if (reduceMotion || typeof window.requestAnimationFrame !== "function") {
      if (serviceTransportVehicle) serviceTransportVehicle.style.opacity = "1";
      positionServiceTransportVehicle(0.68);
      return;
    }

    serviceTransport.classList.remove("is-complete", "is-running");
    void serviceTransport.getBoundingClientRect();
    serviceTransport.classList.add("is-running");
    if (serviceTransportVehicle) serviceTransportVehicle.style.opacity = "1";
    positionServiceTransportVehicle(0);
    serviceTransportFrame = requestAnimationFrame(animateServiceTransportVehicle);
  };

  serviceTransportButtons.forEach((button, index) => {
    button.addEventListener("click", () => activateServiceTransport(button.dataset.transportKey));
    button.addEventListener("focus", () => activateServiceTransport(button.dataset.transportKey));
    if (finePointer) {
      button.addEventListener("mouseenter", () => activateServiceTransport(button.dataset.transportKey));
    }
    ...
    button.addEventListener("keydown", (event) => {
      ...
      if (!direction) return;
      event.preventDefault();
      const nextIndex = (index + direction + serviceTransportButtons.length) % serviceTransportButtons.length;
      serviceTransportButtons[nextIndex].focus();
    });
  });

  const initialServiceTransportKey = serviceTransportButtons.find((button) => button.getAttribute("aria-pressed") === "true")?.dataset.transportKey
    || serviceTransportButtons[0]?.dataset.transportKey;

  const startServiceTransport = () => {
    if (!reduceMotion) {
      document.body.classList.add("transport-ready");
    }
    requestAnimationFrame(() => activateServiceTransport(initialServiceTransportKey));
  };

  if (reduceMotion || !("IntersectionObserver" in window)) {
    startServiceTransport();
  } else {
    const serviceTransportObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (!entry.isIntersecting) return;
        serviceTransportObserver.disconnect();
        startServiceTransport();
      });
    }, { threshold: 0.35 });
    serviceTransportObserver.observe(serviceTransport);
  }
