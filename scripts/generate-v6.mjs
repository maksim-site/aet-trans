...
const outputDirectory = "v6";
const outputRoot = join(projectRoot, outputDirectory);
const assetDepthOffset = 1;
const versionRoot = join(projectRoot, "v6");
...
const postCoverImage = (post) => {
  ...
  const candidates = [
    post.coverImage,
    localNewsImages[post.slug] ? `news/${localNewsImages[post.slug]}` : "",
    ...(post.images || []).map(newsImageRelativePath),
    ...(post.galleryImages || []),
  ];
...
if (!homeNewsPattern.test(versionHome)) {
  throw new Error("Homepage news markers are missing in v5/index.html");
}
...
function header(depth, active) {
  ...
  <nav class="mobile-menu" id="mobileMenu" aria-label="Мобильная навигация">
    <div class="container mobile-menu-inner">
      ${mobileLinks}
      <a href="${root}en/" lang="en" hreflang="en">English <span aria-hidden="true">→</span></a>
      <a class="mobile-menu-request" href="${root}#request">Оставить заявку <span aria-hidden="true">→</span></a>
  ...
  <footer class="site-footer">
    <div class="container footer-main">
      ...
      <nav class="footer-nav" aria-label="Навигация в подвале">
        <a href="${root}#services">Услуги</a><a href="${root}novosti/">Новости</a><a href="${root}#contacts">Контакты</a><a href="${root}en/" lang="en" hreflang="en">English</a>
      </nav>
    </div>
    ...
    <section class="page-hero${image ? "" : " page-hero-no-media"}">
      <div class="container page-hero-grid">
        <div class="page-hero-copy reveal">
          <a class="page-back-link" href="${root}"><span aria-hidden="true">←</span> На главную</a>${eyebrow ? `
  ...
  return `
    <section class="inner-cta">
      <div class="container inner-cta-grid">
        ...
        <div class="inner-cta-actions"><a class="button button-primary" href="${root}#request">Оставить заявку</a><a class="button button-secondary" href="mailto:info@aet-trans.ru">Написать</a></div>
      </div>
    </section>`;
}

function documentPage({ depth = 1, active, title, description, canonicalPath, main, afterMain = "", bodyClass = "", image = "images/oversize.jpg", preloadMap = false, headStart = "", styleVersion = "20260730-2" }) {
  const root = rootPrefix(depth);
  const assets = assetPrefix(depth);
...
${mapConnectionHints}  <link rel="preload" href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@400;500;600;700&amp;display=swap" as="style">
  ...
  <noscript><link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@400;500;600;700&amp;display=swap" rel="stylesheet"></noscript>
  <link rel="stylesheet" href="${root}styles.min.css?v=${styleVersion}">
</head>
<body class="theme-blue-v3 theme-v4 theme-v5 theme-v6 inner-page${bodyClass ? ` ${bodyClass}` : ""}">
${header(depth, active)}
  <main>${main}</main>${afterMain ? `\n${afterMain}` : ""}
${footer(depth)}
  <script src="${root}vendor/lenis.min.js?v=1"></script>
  <script src="${root}script.js?v=20260730-2"></script>
</body>
</html>
`;
}
...
function newsPage() {
  ...
  const main = `
${pageHero({depth, eyebrow: "", title: "Новости и проекты", intro: "Перевозки, отраслевые события, партнерские проекты и важные этапы работы АЕТ Транс."})}
  ...
  return documentPage({depth, active: "news", title: "Новости | АЕТ Транс", description: "Новости, проекты и архив перевозок АЕТ Транс с 2006 года.", canonicalPath: "novosti/", main, bodyClass: "news-index-page", image: "news/conference-2026.jpg"});
}
...
${pageHero({depth, eyebrow: "Санкт-Петербург", title: "Свяжитесь с нами", intro: "Обсудим груз, маршрут, сроки и документы. Можно позвонить или написать на почту."})}
    <section class="section contact-page"><div class="container contact-page-grid"><div class="contact-details"><div class="contact-phone"><span>Телефон</span><strong>+7 (812) 309-46-25</strong><a class="contact-call-mobile" href="tel:+78123094625">Позвонить</a></div><a href="mailto:info@aet-trans.ru"><span>Email</span><strong>info@aet-trans.ru</strong></a><div><span>Офис</span><strong>198035, Санкт-Петербург,<br>Межевой Канал, д. 5, лит. АХ, офис 406</strong></div><div><span>Юридический, почтовый и фактический адрес</span><strong>Совпадают</strong></div></div><div class="contact-page-map"><iframe title="Офис АЕТ Транс на карте" src="https://yandex.ru/map-widget/v1/?ll=30.249529%2C59.911572&amp;z=17&amp;pt=30.249529%2C59.911572,pm2blm" loading="eager" allowfullscreen></iframe></div></div></section>`;
...
function privacyPage() {
  ...
  const root = rootPrefix(depth);
  const main = `
    <section class="legal-header"><div class="container"><a class="page-back-link" href="${root}"><span aria-hidden="true">←</span> На главную</a><p class="context-line">Документы</p><h1>Политика обработки персональных данных</h1><p>Правила работы с данными, которые пользователь передает при обращении в компанию.</p></div></section>
...
function notFoundPage() {
  ...
  return documentPage({depth, title: "Страница не найдена | АЕТ Транс", description: "Страница не найдена.", canonicalPath: "404/", main, headStart, styleVersion: "20260730-2"});
}
...
await Promise.all([
  ...
  writeOutput("negabaritnye-perevozki/index.html", redirectPage("../#services")),
  writeOutput("proekty/index.html", redirectPage("../novosti/")),
  writeOutput("o-kompanii/index.html", redirectPage("../#company")),
  writeOutput("klienty/index.html", redirectPage("../#clients")),
  writeOutput("testimonial/index.html", redirectPage("../#clients")),
  ...
  writeOutput("oversized/index.html", redirectPage("../#services")),
  writeOutput("projects/index.html", redirectPage("../novosti/")),
  writeOutput("about/index.html", redirectPage("../#company")),
  writeOutput("clients/index.html", redirectPage("../#clients")),
  writeOutput("reviews/index.html", redirectPage("../#clients")),
...
console.log(`Generated the v6 preview and ${posts.length} news pages.`);
