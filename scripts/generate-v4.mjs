...
if (!homeNewsPattern.test(versionHome)) {
  throw new Error("Homepage news markers are missing in v4/index.html");
}
...
function documentPage({ depth = 1, active, title, description, canonicalPath, main, afterMain = "", bodyClass = "", image = "images/oversize.jpg", preloadMap = false, headStart = "", styleVersion = "20260727-7" }) {
  const root = rootPrefix(depth);
  const assets = assetPrefix(depth);
...
<body class="theme-blue-v3 theme-v4 inner-page${bodyClass ? ` ${bodyClass}` : ""}">
${header(depth, active)}
  <main>${main}</main>${afterMain ? `\n${afterMain}` : ""}
${footer(depth)}
  <script src="${root}vendor/lenis.min.js?v=1"></script>
  <script src="${root}script.js?v=4"></script>
</body>
</html>
`;
}
...
function newsPage() {
  ...
  const main = `
${pageHero({depth, eyebrow: "", title: "Новости и проекты", intro: "Перевозки, отраслевые события, партнерские проекты и важные этапы работы АЕТ Транс.", image: "news/conference-2026.jpg", imageAlt: "Конференция по проектной логистике"})}
...
${pageHero({depth, eyebrow: "Санкт-Петербург", title: "Свяжитесь с нами", intro: "Обсудим груз, маршрут, сроки и документы. Можно позвонить или написать на почту."})}
    <section class="section contact-page"><div class="container contact-page-grid"><div class="contact-details"><a href="tel:+78123094625"><span>Телефон</span><strong>+7 (812) 309-46-25</strong></a><a href="mailto:info@aet-trans.ru"><span>Email</span><strong>info@aet-trans.ru</strong></a><div><span>Офис</span><strong>198035, Санкт-Петербург,<br>Межевой Канал, д. 5, лит. АХ, офис 406</strong></div><div><span>Юридический, почтовый и фактический адрес</span><strong>Совпадают</strong></div></div><div class="contact-page-map"><iframe title="Офис АЕТ Транс на карте" src="https://yandex.ru/map-widget/v1/?ll=30.249529%2C59.911572&amp;z=17&amp;pt=30.249529%2C59.911572,pm2blm" loading="eager" allowfullscreen></iframe></div></div></section>`;
...
console.log(`Generated the v4 preview and ${posts.length} news pages.`);
