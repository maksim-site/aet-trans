...
if (trustedLogoViewport && trustedLogoTrack) {
  const autoplaySpeed = reduceMotion ? 0 : -36;
  let position = 0;
  ...
  const normalizePosition = () => {
    const loopWidth = trustedLogoTrack.scrollWidth / 2;
    if (!loopWidth) return;
  ...
  const animateTrustedLogos = (time) => {
    ...
    requestAnimationFrame(animateTrustedLogos);
  };
  ...
  const stopTrustedLogoDrag = (event) => {
    ...
    if (trustedLogoViewport.hasPointerCapture(activePointerId)) {
  ...
  window.addEventListener("resize", renderTrustedLogos);
...
if (requestForm) {
  requestForm.addEventListener("submit", (event) => {
    ...
    const subject = encodeURIComponent(`Заявка с сайта АЕТ Транс: ${name}`);
    ...
    if (requestStatus) {
      requestStatus.textContent = "Открываем почтовое приложение для отправки заявки.";
    }

    window.location.href = `mailto:info@aet-trans.ru?subject=${subject}&body=${body}`;
  });
}
